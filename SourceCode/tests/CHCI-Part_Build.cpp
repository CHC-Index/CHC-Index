...
int main(int argc, char** argv){
    ...
    string valid_node_path = argv[cnt++];
...
//            CNNSG = make_unique<cnns_graph<0,224>>() ;
    ...
    CNNSG->Build_Init_part(graph_path, vertex_path, feat_path, edge_path, valid_node_path, Dataset_DT, _metric_, sMethod, com_base
            ,alpha, B, build_L, max_deg, num_threads, activate_rppp);
    std::cout << "Index would be saved into dir: " << index_p << std::endl;
auto s = CLOCK;
    CNNSG->build_all_layer_part();
    auto e = CLOCK;
