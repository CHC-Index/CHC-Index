...
retrieve)
  ...
  Add)
    ...
    ;;
  Del)
    ./Retrieve_Multi_change_num ${DATASET}  ${BASE_INDEX_PATH} ${INDEX_PATH} ${graph}  ${vertex} ${edge} ${data} ${EXP_VALID_NODE_PATH} ${RETRIEVE_RES_PATH} ${RETRIEVE_RES_LOG_PATH} ${QUERY_PATH} \
             ${r} ${LAYER} ${COMBASE} ${SMETHOD} ${METRIC} ${DEFAULT_THREAD_NUM} ${rKACS_out}
...
VAC)
  ...
  echo "./VAC_test ${DATASET}  ${INTEGRATE_INDEX_PATH} ${graph}  ${vertex} ${edge} ${data} ${EXP_VALID_NODE_PATH} ${RETRIEVE_RES_PATH} ${VAC_RES_PATH} ${VAC_RES_PATH_WO}\
                ${LAYER} ${VAC_RUN_WITH} ${VAC_RUN_WO} 1 "
  ./VAC_test ${DATASET}  ${INTEGRATE_INDEX_PATH} ${graph}  ${vertex} ${edge} ${data} ${EXP_VALID_NODE_PATH} ${RETRIEVE_RES_PATH} ${VAC_RES_PATH} ${VAC_RES_PATH_WO} \
              ${LAYER} ${VAC_RUN_WITH} ${VAC_RUN_WO} ${DEFAULT_THREAD_NUM}
  ;;
ATC)
    check_dir_and_make_if_absent_notquit ${ATC_PREFIX}
  echo "./ATC_test ${DATASET}  ${INTEGRATE_INDEX_PATH} ${graph}  ${vertex} ${edge} ${data} ${EXP_VALID_NODE_PATH} ${RETRIEVE_RES_PATH} ${ATC_RES_PATH} ${ATC_RES_PATH_WO}\
              ${LAYER} ${ATC_d} ${ATC_gamma} ${ATC_RUN_WITH} ${ATC_RUN_WO} ${DEFAULT_THREAD_NUM}"
  ./ATC_test ${DATASET}  ${INTEGRATE_INDEX_PATH} ${graph}  ${vertex} ${edge} ${data} ${EXP_VALID_NODE_PATH} ${RETRIEVE_RES_PATH} ${ATC_RES_PATH} ${ATC_RES_PATH_WO}\
              ${LAYER} ${ATC_d} ${ATC_gamma} ${ATC_RUN_WITH} ${ATC_RUN_WO} ${DEFAULT_THREAD_NUM}
  ;;
SEA)
