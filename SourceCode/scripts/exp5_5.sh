...
RPPP=0 # RPPP=1 for optimization 1
SMETHOD=baseline # SMETHOD=opt2 for optimization 2

BUILD_THREAD_NUM=60
INDEX_PREFIX="${root}/indices/${DATASET}"
INDEX_PATH=${INDEX_PREFIX}"/${IDX_ALGO}_L${BUILD_L}_R${DEGREE}_alp${ALPHA}_beta${BETA}_RPPP${RPPP}_SMETHOD${SMETHOD}.chci"
INDEX_LOG_PATH=${INDEX_PREFIX}"/log/${IDX_ALGO}_L${BUILD_L}_R${DEGREE}_alp${ALPHA}_beta${BETA}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
...
RETRIEVE_RES_PATH="${RETRIEVE_RES_PREFIX}${2}_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
RETRIEVE_RES_LOG_PATH="${RETRIEVE_RES_PREFIX}log/${2}_$4_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"

############ VAC ###################
VAC_PREFIX="${root}vac/${EXP_FOLDER_SUFFIX}/${DATASET}/"
VAC_RES_PATH="${VAC_PREFIX}with_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
VAC_RES_PATH_WO="${VAC_PREFIX}wo_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
VAC_RUN_WITH=1
VAC_RUN_WO=0

############ SEA ###################
SEA_PREFIX="${root}sea/${EXP_FOLDER_SUFFIX}/${DATASET}/"
SEA_RES_PATH_WITH="${SEA_PREFIX}with_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
SEA_RES_PATH_WO="${SEA_PREFIX}wo_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
SEA_RUN_WO=1

############ ATC ###################
ATC_d=3 # ATC parameter
ATC_perc=0.50 # ATC parameter
ATC_RUN_WITH=1
ATC_RUN_WO=1

ATC_PREFIX="${root}atc/${EXP_FOLDER_SUFFIX}/${DATASET}/"
ATC_RES_PATH="${ATC_PREFIX}with_r${r}_layer${LAYER}_RPPP${RPPP}_SMETHOD${SMETHOD}.log"
ATC_RES_PATH_WO=${root}atc/${DATASET}/WO/wo_atcperc_${ATC_perc}_atcd_${ATC_d}_RPPP${RPPP}_SMETHOD${SMETHOD}.log
