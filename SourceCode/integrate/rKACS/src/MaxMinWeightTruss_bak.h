...
public:
    ...
    void GetMaxKTrussByIndex_CNNS(int k, const vector<int> &CNNS_Res);
    
    void PrepareQuery_CNNS(int k, vector<int> queryAttrs, const vector<int> &CNNS_Res);
    
    ResultMap rKACS_Incremental_CNNS(int k, int r, vector<int> queryAttrs, const vector<int> &CNNS_Res);
    
    void LoadVertexAttribute_CHCI(string &data_f, parallel_flat_hash_map<int, int> ori_2_cnns_map)
    {
//        FILE* attributeF = fopen(data_f.c_str(), "r");
        std::ifstream in(data_f);
        while (!in.eof()) {
            string line;
            std::getline(in, line);
            ...
            int node;
            sin >> node;
            if (ori_2_cnns_map.count(node)){
                int cur_id = ori_2_cnns_map[node];
                float tmp = 0;
                int cnt = 0;
                while (sin >> tmp){
                    if (tmp > 0){
                        attrsOfNode[cur_id].push_back(cnt);
                        nodesInAttr[cnt].insert(cur_id);
                    }
                    cnt++;
                }
            }
...
//    float Score_Of_Result(_stl res){
//        float score = 10000000;
//        for (auto i = res.begin(); i != res.end(); ++i){
//            for (auto j = i+1; j != res.end(); ++j){
//                float cur_score = (nodeComAttrCnt.at(*i)+nodeComAttrCnt.at(*j)) * set_intersection(attrsOfNode.at(*i)->;
//            }
//        }
//    }
};
