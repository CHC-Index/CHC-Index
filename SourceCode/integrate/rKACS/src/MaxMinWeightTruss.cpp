...
int MaxMinWeightTruss::LoadSimilarity(const char* fileName)
{
	if (fileName != NULL)
	{
		...
		{
//            for (auto ii : attrsOfNode.at(29713)){
//                std::cerr <<  ii ;
//
//            }
...
void MaxMinWeightTruss::ComputeWeight()
{
	...
	for (int m = 0; m < nodeCnt; m++)
	{
		...
		for (int i = 0; i < uDeg; i++)
		{
			...
			int comAttrCnt = uComAttrCnt + nodeComAttrCnt.at(nbr); // A(u) \inter Qw + A(v) \inter Qw
...
void MaxMinWeightTruss::GetMaxKTrussByIndex_CNNS(int k, const vector<int>& CNNS_Res)
{
    vector<int > a;
    int trussValueCnt = trussnessToEdges.size();
    int nodeCnt = 0;
...
//    }
    ...
    //  加入未确认的nodes
    nodeCnt = CNNS_Res.size();
    for (auto node : CNNS_Res){
        maxKTruss->AddNodeUnchecked(node);
    }
    ...
    for (int i = k; i < trussValueCnt; i++)
    {
        ...
        {
            int u = edge.first, v = edge.second;
            if (std::find(CNNS_Res.begin(), CNNS_Res.end(), u) != CNNS_Res.end() && std::find(CNNS_Res.begin(), CNNS_Res.end(), v) != CNNS_Res.end())
                maxKTruss->AddEdgeUnchecked(u, v);
            //  是否要加入整个truss的边？
        }
    }
...
void MaxMinWeightTruss::PrepareQuery_CNNS(int k, vector<int> queryAttrs, const vector<int> &CNNS_Res)
{
    maxKTruss->Clr();
//    GetMaxKTrussByIndex_CNNS(k, CNNS_Res);
    GetMaxKTrussByIndex(k);
...
//    sort(nodes.begin(), nodes.end());
    vector<int> nodes;
    nodes.reserve(CNNS_Res.size());
    for (auto& node_cnt : CNNS_Res)
    {
//        try {
//            if ( nodeComAttrCnt.at(node_cnt) > 0)
//                nodes.push_back(node_cnt);
//        } catch ()
        if (nodeComAttrCnt.contains(node_cnt) &&  nodeComAttrCnt.at(node_cnt) > 0)
        {
            nodes.emplace_back(node_cnt);
        }
    }
    maxKTruss = GetSubGraph(maxKTruss, nodes);
    ...
    for (auto& e : edges)
    {
        //  这里只删除similarity小的边, 对不在CNNS_Res中的边没影响
        maxKTruss->DelEdge(e.first, e.second);
    }
...
ResultMap MaxMinWeightTruss::rKACS_Incremental_CNNS(int k, int r, vector<int> queryAttrs, const vector<int>& CNNS_Res)
{
    PrepareQuery_CNNS(k, queryAttrs, CNNS_Res);
