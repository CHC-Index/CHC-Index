...
namespace chci {
 ...
 // 为了增删改实验引入的数据结构
        ...
        virtual void Save_Graph(const std::string &cnns_index_output) = 0;
        ...
        virtual CSTATUS Prune_Dataset(CCORE coreness, std::string &folder, std::string &ds_name) = 0;
...
virtual    void
  Load_Graph_bin_corelist(Dataset_Type ds_t, __Metric mtc, Cmodel cmodel,
                          StructureMethod sMeth, const std::string &cnns_index_input, const std::string &valid) = 0 ;
virtual void Load_Graph_Topology(const std::string &graph_path,
    ...
    class cnns_graph : public CHCIndex {
    public:
        cnns_graph() = default;
...
#pragma omp parallel for
        ...
        cnns_graph(const std::string &graph_path,
                   const std::string &vertex_path,
                   const std::string &data_path,
                   const std::string &edge_path,
                   Dataset_Type speci_dataset,
        ...
        void Begin_Build_Graph(const std::string &graph_path,
            ...
            specific_dataset = speci_dataset;
            read_adj *ptmp_graph = new read_adj(graph_path, vertex_path, specific_dataset, edge_path);
            ...
            if (Paper_B == 0) {
                ...
                if (Paper_B * Paper_B - Paper_A < 0) {
                    throw std::runtime_error("Beta is not valid!!!");
                }
                Paper_rp_lb = Paper_B - std::sqrt(Paper_B * Paper_B - Paper_A);
                Paper_rp_hb = Paper_B + std::sqrt(Paper_B * Paper_B - Paper_A);
            }
...
//                                       neighbor_candi);
//                Robust_Prune_Build(cnns_id, cur_layer, cur_lock_num, neighbor_candi, cur_layer_adj[i], 1);
...
#pragma omp parallel for num_threads(numThreads) schedule(dynamic)
        ...
        maxHeap<CDIST>
        Return_NeighborCandi(CNODEID cnns_id, CCORE cur_coreness, size_t cur_compo) {
            maxHeap<CDIST> neighbor_candi;
            for (auto i = 0; i < Return_Adj_Track(cnns_id).size(); ++i) {
                auto cur_common_layer = Return_Adj_Track(cnns_id).first;
                if (cur_common_layer == cur_coreness - 1) {
                    auto cur_nei = Return_Adj_Track(cnns_id)[i].second;
                    for (auto &j: Return_Adj_Track(cur_nei)) {
                        if (j.first >= cur_coreness - 1 && compo_l[j.second][cur_coreness] == cur_compo) {
                            neighbor_candi.emplace(-Cal_Dist(cnns_id, j.second), j.second);
                        }
                    }
                }
            }
...
//            std::cerr << hop_2_dist.size();

            auto hop_2_sdist1 = [this](int hop) { return 1.0 * sdist_scale_factor * (hop - 1); };
            int hop = 1;
...
//            if (search_queue_length < node_num_per_compo[cur_coreness][cur_component]*1.2*r)
            search_queue_length = 1.2 * node_num_per_compo[cur_coreness][cur_component] * r;
        std::cerr << node_num_per_compo[cur_coreness][cur_component] << std::endl;
...
std::vector<CNODEID> Anonymous_Search_nohop(CNODEID p, float r, CCORE cur_coreness, size_t *time) {
            maxHeap<CDIST> neighbor_candi;
...
//            if (search_queue_length < node_num_per_compo[cur_coreness][cur_component]*1.2*r)
        ...
        std::cerr << node_num_per_compo[cur_coreness][cur_component] << std::endl;
            ...
            if (core_list[sp_2] >= cur_coreness){
            expandset.emplace(-paper_dist(sp_2), sp_2);
            vis[sp_2] = true;
            }
            resultset.emplace(0, sp_2);
            ...
            while (!cand_set.empty()) {
                ...
                for (auto j = 0; j < cur_neigh_data_l.size(); ++j) {
                    ...
                    if (dist * alp < -cur.first) {
                        ...
                        if (x > Paper_rp_hb || x < Paper_rp_lb) {
                            good = false;
                            recycleBin.emplace(cur);
                            break;
                        }
...
#pragma omp critical
            ...
            for (auto i = 0; i < std::min(20,layer_num); ++i) {
                std::cout << "Current coreness " << i << " has " << splist_num[i] << " components.\n";
            }
...
// 增：一次加入一批顶点
        ...
        std::cerr << "Before Add: " << node_num_per_compo[15][0] <<std::endl;
//            int cnt = 0;

//            for (auto ori_id: need_add) {
#pragma omp parallel for schedule(dynamic) num_threads(20)
                for (auto i = 0; i < need_add.size(); ++i) {
                    auto ori_id = need_add[i];
                Add_Node(ori_id);
                if (i % 1000 == 0){
                    std::cout << "Added " << i << " Nodes\n";
                }
            }
        std::cerr << "After Add: " <<node_num_per_compo[15][0] <<std::endl;
            return 0;
        }
            ...
            if (ori_2_cnns_map.contains(ori_id)) {
                ...
                for (auto layer = 0; layer <= cur_coreness; ++layer) {
                    ...
                    for (auto nei: cur_graph_ori_adj[cnns_id]) {
                        //  识别所有coreness为0的邻居，用自己的coreness更新其coreness
//                        if (core_list[nei] == 0){
//                            isolate_nei.emplace_back(nei);
//                        }
...
//                CNODEID cur_cnns_id = ntotal_++;
//                ori_2_cnns_map[ori_id] = cur_cnns_id;
//                cnns_2_ori.emplace_back(ori_id);
//                Structure_data cur_structData{cur_cnns_id, neighbor_in, cnns_2_reordered};
//                struct_data_.emplace_back(cur_structData);
//                data_.emplace_back(generalData);
//                CCORE cur_coreness = App_Coreness(neighbor_in);
//                core_list.emplace_back(cur_coreness);
            }
...
//                        for (int i = common_nei_trussness.size()-1; i >=0; --i) {
                            if (cnt >= *i){
                                trussness = cnt;
                                break;
                            } else {
...
// 处理数据集，决定需要保留的顶点ID
// 输出一些顶点ID，包括删除的什么顶点，留下的什么顶点
        CSTATUS Prune_Dataset(CCORE coreness, std::string &folder, std::string &ds_name) {
            //  删除coreness以下的顶点
            std::cout << "Pruning!!!\n";
//            std::vector<float> del_percent{5, 10, 15, 20, 25, 30};
            std::vector<float> del_percent{1, 2, 4, 8, 16, 32};
            std::vector<std::vector<CNODEID>> del_nodes(del_percent.size());
            std::vector<std::vector<CNODEID>> del_nodes_full(del_percent.size()); // 每个数组中存放完整的已删除顶点


            maxHeap<int> random_node_heap;
            ...
            for (auto i = 0; i < ntotal_; ++i) {
                if (core_list[i] < coreness) {
                    random_node_heap.emplace(distrib(gen), cnns_2_ori[i]);
                }
            }
            for (auto i = 0; i < del_percent.size(); ++i) {
                int cur_insert_num = ntotal_ / 100;
                if (i > 0) {
                    cur_insert_num = (del_percent[i] - del_percent[i - 1]) / 100 * ntotal_;
                }
                del_nodes[i].reserve(cur_insert_num);
                if (random_node_heap.size() < cur_insert_num) {
                    std::cerr << "Current percent: " << i << "\n";
                    throw (std::runtime_error("No enough Nodes!!!!!!\n"));
                }
                for (auto j = 0; j < cur_insert_num; ++j) {
                    del_nodes[i].emplace_back(random_node_heap.top().second);
                    random_node_heap.pop();
                }
                del_nodes_full[i] = del_nodes[i];
                if (i > 0)
                    del_nodes_full[i].insert(del_nodes_full[i].end(),del_nodes[i-1].begin(), del_nodes[i-1].end());
                std::sort(del_nodes[i].begin(), del_nodes[i].end());
                std::sort(del_nodes_full[i].begin(), del_nodes_full[i].end());
            }

            std::vector<CNODEID> all_ori_id = cnns_2_ori;
            std::sort(all_ori_id.begin(), all_ori_id.end());
            std::vector<std::vector<CNODEID >> remain_nodes(del_percent.size());
            std::set_difference(all_ori_id.begin(), all_ori_id.end(), del_nodes[0].begin(), del_nodes[0].end(),
                                std::back_inserter(remain_nodes[0]));
            for (auto i = 1; i < del_percent.size(); ++i) {
                std::set_difference(remain_nodes[i - 1].begin(), remain_nodes[i - 1].end(), del_nodes[i].begin(),
                                    del_nodes[i].end(),
                                    std::back_inserter(remain_nodes[i]));
            }

            for (auto i = 0; i < del_percent.size(); ++i) {
                std::string out_nodes =
                        folder + "/" + ds_name + "_remain_" + std::to_string(static_cast<int>(del_percent[i])) +
                        "%.txt";
                std::ofstream outs(out_nodes);
                for (auto node: remain_nodes[i]) {
                    outs << node << "\n";
                }
                outs.close();
                outs.clear();
                std::string out_del_nodes =
                        folder + "/" + ds_name + "_del_" + std::to_string(static_cast<int>(del_percent[i])) + "%.txt";
                outs.open(out_del_nodes);
                for (auto node: del_nodes_full[i]) {
                    outs << node << "\n";
                }
                outs.close();
            }
...
#define SWITCH_DSTYPE(in, str) if (in == TOSTRING(str)) specific_dataset = str
#define SWITCH_SMETHOD(in, str) if (in == TOSTRING(str)) sMethod_ = str
#define SWITCH_COMBASE(in, str) if (in == TOSTRING(str)) communityBase = str

        void Load_Graph_Index(const std::string &graph_path,
                              const std::string &vertex_path,
                              const std::string &data_path,
                              const std::string &edge_path,
                              const std::string &Index_path) {
            std::cout.setstate(std::ios::failbit);
            std::ifstream indexIn(Index_path);
            if (!indexIn.is_open()) {
                std::cerr << "Failed to open the index: " << Index_path << ", please check if it exists.\n";
                return;
            }
            std::string index_dataset;
            std::getline(indexIn, index_dataset);
            SWITCH_DSTYPE(index_dataset, DBLP);
            SWITCH_DSTYPE(index_dataset, small_dblp);
            SWITCH_DSTYPE(index_dataset, IMDB_Person);
            SWITCH_DSTYPE(index_dataset, IMDB_Movie);
            SWITCH_DSTYPE(index_dataset, FourSquare);
            SWITCH_DSTYPE(index_dataset, CORA);
            SWITCH_DSTYPE(index_dataset, Twitch);
            SWITCH_DSTYPE(index_dataset, LiveJournal);
            SWITCH_DSTYPE(index_dataset, Facebook);
            SWITCH_DSTYPE(index_dataset, DBpedia_Basketballplayer);
            SWITCH_DSTYPE(index_dataset, DBpedia_Automobile);
            SWITCH_DSTYPE(index_dataset, DBpedia_Soccerplayer);
            SWITCH_DSTYPE(index_dataset, FB_MERGE_ORI);
            SWITCH_DSTYPE(index_dataset, FB_MERGE);
            std::getline(indexIn, index_dataset);
            SWITCH_SMETHOD(index_dataset, baseline);
            SWITCH_SMETHOD(index_dataset, A);
            SWITCH_SMETHOD(index_dataset, B);
            std::getline(indexIn, index_dataset);
            SWITCH_COMBASE(index_dataset, core);
            SWITCH_COMBASE(index_dataset, truss);

            read_adj *ptmp_graph = new read_adj(graph_path.c_str(), vertex_path.c_str(), specific_dataset,
                                                edge_path.c_str());
            ntotal_ = ptmp_graph->total_node_num;
            ms_.datasetType = specific_dataset;
            ...
            std::string tmp_str;
            std::getline(indexIn, tmp_str);
            tmp_str.erase(std::remove_if(tmp_str.begin(), tmp_str.end(), [&](const auto &item) {
                return ((item < '0' || item > '9') && (item != ' '));
            }), tmp_str.end());
            std::istringstream in(tmp_str);
            in >> max_degree;
            in >> build_queue_length;
            in >> numThreads;
            in >> max_coreness;
            in >> layer_num;
            in >> ntotal_;
            std::getline(indexIn, tmp_str); //  Start Pt List
            sp_l_list.resize(layer_num);
            core_list.resize(ntotal_);
            deg_list.resize(layer_num, max_degree);
            ...
            for (auto i = 0; i < layer_num; ++i) {
                ...
                std::getline(indexIn, tmp_str);
                in.str(tmp_str);
                std::vector<CNODEID> cur_layer_enter_pt;
                CNODEID tmp_id;
                in >> tmp_id;
                assert(tmp_id == i);
                CNODEID tmp_compo;
                in >> tmp_compo;
                cur_layer_enter_pt.reserve(tmp_compo); //  Reserve enough space to store enter pt
                while (in >> tmp_id) {
                    cur_layer_enter_pt.emplace_back(tmp_id);
                }
                sp_l_list[i] = cur_layer_enter_pt;
            }
            ...
            for (auto i = 0; i < ntotal_; ++i) {
                ...
                assert(cnns_id == i);
                in >> ori_id;
                auto &compo = compo_l[cnns_id];
                compo.resize(core_list[i] + 1, -1);
                for (auto j = 0; j <= core_list[i]; ++j) {
                    in >> compo[j];
                }

                std::vector<std::pair<CLAYER, CNODEID>> &cur_track = Return_Adj_Track(cnns_id);
                CNODEID cur_nei = -1;
                CLAYER cur_max_common_layer = -1;
                while (in >> cur_nei) {
                    in >> cur_max_common_layer;
                    cur_track.emplace_back(cur_max_common_layer, cur_nei);
                }
            }
...
#undef SWITCH_DSTYPE
#undef SWITCH_SMETHOD
#undef SWITCH_COMBASE

        void Save_Graph(const std::string &cnns_index_output) {

            std::ofstream indexOut(cnns_index_output);

            indexOut << Print_Dataset_Type(specific_dataset) << std::endl;
            indexOut << Print_Structure_Method(sMethod_) << std::endl;
            indexOut << Print_Community_Base(communityBase) << std::endl;
            indexOut << "Degree : " << max_degree
                     << " Build_L: " << build_queue_length
                     << " Thread: " << numThreads
                     << " Max coreness: " << layer_num - 1
                     << " Layer num: " << layer_num
                     << " Vertex num: " << ntotal_
                     << std::endl;
            indexOut << "Start Point List : " << std::endl;
            for (CNODEID i = 0; i < layer_num; ++i) {
                indexOut << i << " ";
                indexOut << sp_l_list[i].size() << " ";
                for (auto j = 0; j < sp_l_list[i].size(); ++j) {
                    indexOut << std::to_string(sp_l_list[i][j]) << " ";
                }
                indexOut << std::endl;
            }
            for (CNODEID i = 0; i < ntotal_; ++i) {
                indexOut << std::to_string(core_list[i]);
                indexOut << " ";
            }
            indexOut << std::endl;
            //  存放邻接表
            //  Store the whole compressed adjacency
            for (CNODEID i = 0; i < ntotal_; ++i) {
                std::string cur_line = "";

                //  存放cnns_id 与origin_id
                cur_line += std::to_string(i) + " " + std::to_string(cnns_2_ori[i]) + " ";

                //  存放component
                for (auto j: compo_l[i]) {
                    cur_line += std::to_string(j) + " ";
                }

                //  存放邻居及极大层
                for (auto j : Return_Adj_Track(i)) {
        cur_line +=
            std::to_string(j.second) + " " + std::to_string(j.first) + " ";
      }
      indexOut << cur_line;
      indexOut << std::endl;
    }
...
#define OUT(stream, _str)                                                      \
  ...
  Load_Graph_bin(Dataset_Type ds_t, __Metric mtc, Cmodel cmodel,
    ...
    specific_dataset = ds_t;
    communityBase = cmodel;
    auto *ptmp_graph = new read_adj(graph_path.c_str(), vertex_path.c_str(),
    ...
    void Load_Graph_bin_corelist(Dataset_Type ds_t, __Metric mtc, Cmodel cmodel,
                                 StructureMethod sMeth, const std::string &cnns_index_input, const std::string &valid_node_path) {
    std::ifstream indexIn(cnns_index_input, std::ios::in | std::ios::binary);
    specific_dataset = ds_t;
    communityBase = cmodel;
...
//            std::set<CNODEID > valid_node;
  ...
  inline const CCORE Get_Cur_Compo(CNODEID cnns_id, CCORE coren) {
    return compo_l[cnns_id][coren];
  }
};
} // namespace cnns

#endif // CS_ON_ANNS_CNNS_GRAPH_H
